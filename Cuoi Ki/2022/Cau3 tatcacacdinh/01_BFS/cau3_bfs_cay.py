...
def bfs_to_zero_heuristic_goals(graph, start):
    queue = deque([start])
    visited = {start}
    parent = {start: None}
    exploration_order = []
...
def format_steps(steps):
    headers = ["Buoc", "Queue truoc", "Lay ra", "Them vao", "Queue sau"]
    rows = []
    ...
    for index, step in enumerate(steps, start=1):
        ...
        rows.append([
            index,
            format_list(step["queue_before"]),
            f"{step['current']}(h={step['h']})",
            added,
            format_list(step["queue_after"]),
        ])

    return "\n".join([
        "Bang trang thai:",
        make_table(headers, rows),
        "Ghi nhan moi dinh co h = 0 va tiep tuc cho den khi queue rong.",
    ])


def solve():
    graph = build_graph(V, E, H)
    goals, exploration_order, steps = bfs_to_zero_heuristic_goals(graph, "S")
    lines = [
        "THUAT TOAN BFS",
