...
Với bài lâu đài tìm đường thoát, em đã cài đặt 5 thuật toán phù hợp. Riêng các thuật toán dùng heuristic là A* và Greedy Best-First Search, em dùng `h(x)` là khoảng cách Manhattan từ ô hiện tại đến rìa gần nhất.
...
|---|---|---|---|---:|---:|---|
| A* | `01_AStar` | `A_out.csv` | 7 | 11 | Dùng `f(x) = g(x) + h(x)`, với `h(x)` Manhattan |
...
| Greedy Best-First | `04_Greedy_BestFirst` | `Greedy_out.csv` | 7 | 9 | Chọn ô có `h(x)` Manhattan nhỏ nhất |
| UCS | `05_UCS` | `UCS_out.csv` | 7 | 16 | Chọn ô có `g(x)` nhỏ nhất |

## Cấu trúc thư mục

```text
BFS_DFS_Astar_Manhattan_Cau1/
├── A_in.csv
...
python "2025/De1/BFS_DFS_Astar_Manhattan_Cau1/02_BFS/cau1_bfs.py"
