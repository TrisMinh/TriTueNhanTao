...
Với từng mẫu dữ liệu, vì `y` là one-hot nên chỉ phần xác suất của lớp đúng ảnh hưởng trực tiếp đến loss. Nếu xác suất của lớp đúng càng gần 1 thì `log(y_hat)` càng gần 0, làm loss nhỏ. Nếu xác suất của lớp đúng càng gần 0 thì `log(y_hat)` là số âm rất lớn, làm loss tăng mạnh.
...
Trong đó `m` là số mẫu dữ liệu. Dữ liệu gốc có 75 mẫu; sau data augmentation, tập huấn luyện có `m = 225` mẫu.
...
-> tính Cross-Entropy Loss khi huấn luyện
