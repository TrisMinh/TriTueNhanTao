...
def main():
    ...
    parameters, history = train_neural_network(X_train_scaled, y_train)
    ...
    save_feature_scatter_chart(X_train, y_train, X_output, output_labels, current_dir / "nn_feature_scatter.png", "Neural Network - Phan bo du lieu")
...
Chương trình tự cài đặt đầy đủ các bước:

1. Đọc dữ liệu CSV.
2. Mã hóa nhãn hoa thành số.
3. Chuẩn hóa 4 đặc trưng.
4. Khởi tạo trọng số.
5. Lan truyền tiến để tính xác suất.
6. Tính Cross-Entropy Loss.
7. Lan truyền ngược để tính gradient.
8. Cập nhật trọng số bằng Gradient Descent.
9. Dự đoán 30 mẫu trong `output_2.csv`.
...
![Neural Network loss](nn_loss.png)

Confusion matrix:

![Neural Network confusion matrix](nn_confusion_matrix.png)

Biểu đồ phân bố dữ liệu:

![Neural Network scatter](nn_feature_scatter.png)
